...
public class FlowRestController {
	...
	@ResponseBody
	public RestResult instances(@RequestParam String loginToken, UriComponentsBuilder ucb) {
		String userId = Constants.LOGIN_TOKEN_CACHE.get(loginToken);
		DataResult result = restTemplate.getForObject(ucb.scheme("http").path("/service/runtime/process-instances").queryParam("participate", userId).build().toUriString(), DataResult.class);
		return RestResult.success(result.getData());
	}
