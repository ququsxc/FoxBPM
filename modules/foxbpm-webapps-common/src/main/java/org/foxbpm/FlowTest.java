...
public class FlowTest {
	...
	public static void sendMsg(ProcessInstance processInstance, String userId, String taskNodeId) {
		// 获取当前任务处理人
		...
		// Task task =
		// engine.getTaskService().createTaskQuery().processInstanceId(processInstance.getId())
		...
		// TxtMessage messagePublishPrivateVoiceMessage = new
		// TxtMessage("请尽快处理报销审批审批", "您好！请尽快处理报销审批审批");
		// CodeSuccessReslut messagePublishPrivateResult = null;
		// try {
		// messagePublishPrivateResult =
		// rongCloud.message.publishPrivate(userId,
		// messagePublishPrivateToUserId, messagePublishPrivateVoiceMessage,
		// "thisisapush", "{\"pushData\":\"hello\"}", "4", 0, 0, 0);
		// System.out.println("publishPrivate: " +
		...
		// TxtMessage messagePublishDiscussionTxtMessage = new
		// TxtMessage("打雷啦，下雨收衣服啊~~", "打雷啦，下雨收衣服啊~~");
		// CodeSuccessReslut messagePublishDiscussionResult = null;
		// try {
		// messagePublishDiscussionResult =
		// rongCloud.message.publishDiscussion(userId,
		// "cc1123e8-5e71-4949-b58d-4a747780deb3",
		...
		TxtMessage messagePublishGroupTxtMessage = new TxtMessage("这是一条群消息！", null);
		try {
			CodeSuccessReslut messagePublishGroupResult = rongCloud.message.publishGroup(userId, messagePublishGroupToGroupId, messagePublishGroupTxtMessage, null, null, null, null);
		} catch (Exception e) {
	...
	public static ProcessInstance getInstance(ProcessInstance processInstance) {
		// 获取当前流程实例流程变量
		// List<VariableInstanceEntity> entities = ((ProcessInstanceEntity)
		// processInstance).getDataVariableMgmtInstance()
			...
			for (Iterator iterator = instances.iterator(); iterator.hasNext();) {
				...
				if (processInstance2.getProcessDefinitionKey().equals("tt_1")) {
					...
					for (Iterator iterator2 = variableInstanceEntities.iterator(); iterator2.hasNext();) {
						...
						// 找到属于当前实例的流程变量
						if (variableInstance.getProcessInstanceId().equals(processInstance2.getId()) && variableInstance.getKey().equals("account") && "1".equals(variableInstance.getValueObject())) {

							// 获取实例的所有任务
							List<Task> tasks = engine.getTaskService().createTaskQuery().processInstanceId(processInstance2.getId()).list();
	...
	// 集成‘即时计算’
	public static void startFlow(ProcessInstanceEntity instance, Connection conn) {
		// TODO Auto-generated method stub
		for (int i = 0; i < 100; i++) {
			if (i == 49) {
				break;
