	...
	public DataResult getProcessInstance() {
		...
		if (mapList != null && mapList.size() > 0) {
			for (Map<String, Object> tmp : mapList) {
				...
				String processLocationString = LocationUtil.parseProcessLocation(processLocation, getQueryParameter(RestConstants.PARTICIPATE, queryForm));

				String[] arr = processLocationString.split("##");
				if (arr.length == 2) {
					tmp.put("processLocationString", arr[0]);
					String nodeId = arr[1];
					TaskService taskService = FoxBpmUtil.getProcessEngine().getTaskService();
					List<Task> tasks = taskService.createTaskQuery().processInstanceId(StringUtil.getString(tmp.get("id"))).nodeId(nodeId).list();
					tmp.put("formUriView", tasks.get(0).getFormUriView());
				} else {
